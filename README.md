...
- **Crosspoint matrix.** Click a sender and a receiver to connect them according to NMOS IS-05. Autotake or "stage and then TAKE" workflow. Each sender and receiver shows its transport: ST 2110 (split in two tones for ST 2022-7) or MXL. Only senders and receivers of the same transport family can be crosspointed. A receiver connected by transport file alone, with no sender id in IS-04 or IS-05, is matched to its sender by multicast group, port and source; an MXL receiver by the flow it reads. When an MXL sender moves to another flow or domain while it runs, the MXL receivers taking it are pointed at the new one; a sender that stops naming a flow leaves them as they are. The Show menu filters by availability, hidden state, essence and transport. Devices are listed by their crosspoint number on both axes and on the Details page, a node at the lowest number of its devices; devices without one follow in natural name order. New devices are numbered from `firstDynamicNumber` (default 1000) on, and the number is changed on the Details page: a number already in use swaps with that device.
